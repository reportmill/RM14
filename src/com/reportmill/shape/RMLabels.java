...
public void paintShape(RMShapePainter aPntr)
{
    ...
    // Draw frame rect
    aPntr.setColor(Color.darkGray); aPntr.setStroke(RMAWTUtils.Stroke1);
    aPntr.draw(bounds);
    ...
    for(int i=0; i<getNumberOfRows(); i++)
        for(int j=0; j<getNumberOfColumns(); j++) {
            ...
            RMRect labelRect = new RMRect(labelX, labelY, label.getWidth(), label.getHeight()); // Get label rect
            aPntr.setColor(i==0 && j==0? new Color(.45f, 0, 0) : Color.black); // Set stroke red for first
            aPntr.draw(labelRect); // Draw label rect
        }
}
