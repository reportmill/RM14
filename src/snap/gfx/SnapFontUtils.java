...
public class SnapFontUtils {
    ...
    // A list of all fonts
    static List <SnapFont>  _altFonts;
...
public static synchronized String[] getFontNames()
{
    ...
    Font fonts[] = SnapFontUtils.getFonts();
...
{
    if(_familyNames==null) getFontNames(); // If family names haven't been loaded, call allFontNames
    return _familyNames;
}
...
public static Font guessFont(String aName)
{
    ...
    if(guessFont==null) {
        ...
        if(!name.equals(getFontNameNormalized(guessFont.getFontName())) &&
           ...
           System.out.println("SnapFontUtils: Couldn't find font for " + aName + " (using " + guessFont.getFontName() + ")");
    }
 ...
 */
public static SnapFont getAltFont(char aChar)
{
    // Iterate over alternate fonts and return first that can display
    for(int i=0, iMax=getAltFonts().size(); i<iMax; i++) { SnapFont font = getAltFonts().get(i);
 ...
 */
public static List <SnapFont> getAltFonts()  { return _altFonts!=null? _altFonts : (_altFonts=createAltFonts()); }

/**
 * Returns the list of suggested alternate fonts.
 */
static List <SnapFont> createAltFonts()
{
    // Yuichi recommended:
    ...
    //for(int i=0; i<names.length; i++) { SnapFont f = SnapFont.getFont(names[i], 12, false);
    ...
    Font fonts[] = SnapFontUtils.getFonts();
    List <SnapFont> altFonts = new Vector(fonts.length);
    for(int i=0; i<fonts.length; i++)
        altFonts.add(new SnapFont(fonts[i].getFontName(), 12));
    
    // Make sure Symbol is the first font and return
    altFonts.add(0, new SnapFont("Symbol", 12));
    return altFonts;
}
...
public static void startFontLoadingInBackground()
{
    ...
    // Catch exceptions
    catch(Exception e) { System.err.println("SnapFontUtils:InitFonts: Error loading system fonts"); }
}
