...
public class RMCrossTabCell extends RMTextShape {
    ...
    static Insets _defaultMargin = new Insets(5, 5, 5, 5);

    // Default cell paragraph (aligned center)
    static RMParagraph _defaultCellParagraph = RMParagraph.DEFAULT.deriveAligned(RMTypes.AlignX.Center);

/**
 * Creates a new RMCrossTabCell.
 */
public RMCrossTabCell()  { setAlignmentY(RMTypes.AlignY.Middle); }
...
public RMXString createXString()
{
    return new RMXString() {
        public RMParagraph getDefaultParagraph() { return _defaultCellParagraph; }
    };
}
...
public void paintShape(RMShapePainter aPntr)
{
    ...
    if(aPntr.isEditing() && (isRowHeader() || isColumnHeader())) {
        double x = Math.round(getMaxX()) - getX();
        double y = Math.round(getY()) - getY();
        aPntr.setColor(Color.black);
        aPntr.setFont(RMFontUtils.getFont("Arial", 8));
...
public XMLElement toXML(XMLArchiver anArchiver)
{
    ...
    if(getAlignmentY()==RMTypes.AlignY.Middle) e.removeAttribute("valign");
        
    // Override RMText vertical alignment: if is RMText's default, ALIGN_TOP, add attribute
    else if(getAlignmentY()==RMTypes.AlignY.Top)
