...
{
    if(SnapUtils.equals(aName, _name)) return;  // If no change, just return
    firePropertyChange("Name", _name, _name = aName, -1);  // Set value and fire PropertyChange
}
 ...
 */
public void setSite(WebSite aSite)  { _site = aSite; }
...
public void addEntity(Entity anEntity, int anIndex)
{
    ...
    // Fire PropertyChange
    firePropertyChange("Entity", null, anEntity, anIndex);
}
...
public Entity removeEntity(int anIndex)
{
    ...
    firePropertyChange("Entity", entity, null, anIndex);
    return entity;
}
