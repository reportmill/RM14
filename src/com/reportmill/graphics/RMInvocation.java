 ...
 */
public Class getArgClass()  { return RMKey.getAccessor(getTarget(), getAction()).getGetMethod().getReturnType(); }

/**
 * Invoke method.
 */
public void invoke()  { RMKeyChain.setValue(getTarget(), getAction(), _args[0]); }
...
public RMInvocation getInterpolation(double aFraction, RMInvocation other)
{
    ...
    if(RMMath.equals(aFraction, 0)) return this;
    if(RMMath.equals(aFraction, 1)) return other;
    ...
    if(float.class.isAssignableFrom(argClass) || double.class.isAssignableFrom(argClass)) {
        double v1 = RMUtils.doubleValue(getArg());
        double v2 = RMUtils.doubleValue(other.getArg());
    ...
    // Handle RMXString
        ...
        RMXString s1 = (RMXString)getArg();
        RMXString s2 = (RMXString)other.getArg();
        invocation.setArg(RMXStringUtils.blend(s1, s2, (float)aFraction));
    }
