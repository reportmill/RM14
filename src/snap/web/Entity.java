 ...
 */
public class Entity extends SnapObject implements PropertyChangeListener,JSONArchiver.GetKeys,XMLArchiver.Archivable {
...
public void setName(String aName)
{
    ...
    if(SnapUtils.equals(name, _name)) return; // If no change, just return
    firePropertyChange(Name_Prop, _name, _name = name, -1); // Set value and fire PropertyChange
}
...
public void addProperty(Property aProperty, int anIndex)
{
    ...
    aProperty.addPropertyChangeListener(this);  // Start listening to PropertyChanges
    _attrs = _attrsSorted = _relations = _relationsSorted = _primaries = null;  // Reset cached lists
    
    // Fire property change event
    firePropertyChange("Property", null, aProperty, anIndex);
}
...
public Object removeProperty(int anIndex)
{
    ...
    property.removePropertyChangeListener(this);  // Stop listening to PropertyChanges
    _attrs = _attrsSorted = _relations = _relationsSorted = _primaries = null;  // Reset cached lists

    // Fire property change event and return
    firePropertyChange("Property", property, null, anIndex);
    return property;
}
...
public void setDescriptorKey(String aValue)
{
    ...
    firePropertyChange("DescriptorKey", _descKey, _descKey = aValue, -1);
}
...
/**
 * PropertyChangeListener implementation to forward Property property changes to entity property change listener.
 */
public void propertyChange(PropertyChangeEvent anEvent)  { firePropertyChange(anEvent); }
 ...
 */
public Entity fromBytes(byte theBytes[]) throws JSONReader.RMJSONException
{
    String string = StringUtils.getISOLatinString(theBytes);
    string = string.replace("RMEntity", "Entity").replace("RMProperty", "Property");
    return (Entity)new JSONArchiver().readObject(string, this);
}
