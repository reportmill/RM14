...
{
    // Write bytes
    try { RMFileUtils.writeBytes(new File(aPath), getBytes()); }
    ...
    for(String iname : _files.keySet()) { byte bytes[] = _files.get(iname);
        ...
        try { RMFileUtils.writeBytes(file, bytes); }
        catch(Exception e) { throw new RuntimeException(e); }
    }
}
...
private static class RMShapeHpr <T extends RMShape> extends RMHtmlHelper<T> {
    ...
    protected void writeShape(T aShape, RMHtmlFile aFile, XMLElement anXML)
    {
        ...
        RMEffect effect = aShape.getEffect();
        ...
        if(fill!=null || stroke!=null || effect!=null) {
            ...
            pathXML.add("d", new SVGPathMaker().append(aShape.getPathInBounds()).toString());
    ...
    protected String writeFill(T aShape, RMFill aFill, RMHtmlFile aFile, XMLElement anXML)
    {
        ...
        if(aFill instanceof RMGradientFill) { RMGradientFill gfill = (RMGradientFill)aFill;
            ...
            for(int i=0, iMax=gfill.getColorStopCount(); i<iMax; i++) {
                XMLElement sxml = new XMLElement("stop");
                sxml.add("offset", Math.round(gfill.getStopPosition(i)*100) + "%");
    ...
    /** Writes a fill. */
    protected String writeFilter(T aShape, RMEffect anEffect, RMHtmlFile aFile, XMLElement anXML)
    {
        // Handle RMShadowEffect
        if(anEffect instanceof RMShadowEffect) { RMShadowEffect shadow = (RMShadowEffect)anEffect;
        ...
        // Handle RMBlurEffect
        else if(anEffect instanceof RMBlurEffect) { RMBlurEffect blurEf = (RMBlurEffect)anEffect;
           XMLElement filter = new XMLElement("filter");
           filter.add("id", "filt");
           XMLElement blur = new XMLElement("feGaussianBlur");
    ...
    protected String imageName(RMHtmlFile aWriter, RMShape aShape, RMImageData anImageData)
    {
        ...
        for(String key : aWriter._files.keySet()) {
            ...
            if(RMArrayUtils.equals(imageBytes, bytes))
                return key;
        }
...
private static class SVGPathMaker extends StringMaker {
    ...
    {
        PathIterator pi = aShape.getPathIterator(new AffineTransform());
        double pts[] = new double[6];
        while(!pi.isDone()) {
            switch(pi.currentSegment(pts)) {
                case PathIterator.SEG_MOVETO: moveTo(pts[0], pts[1]); break;
                case PathIterator.SEG_LINETO: lineTo(pts[0], pts[1]); break;
                case PathIterator.SEG_QUADTO: quadTo(pts[0], pts[1], pts[2], pts[3]); break;
                case PathIterator.SEG_CUBICTO: cubeTo(pts[0], pts[1], pts[2], pts[3], pts[4], pts[5]); break;
