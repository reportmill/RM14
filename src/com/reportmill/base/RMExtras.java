...
import com.reportmill.pdf.writer.RMPDFWriter;
    ...
    // Handle Text
        ...
        for(int i=0, iMax=xstring.getRunCount(); i<iMax; i++) { RMXStringRun run = xstring.getRun(i);
            if(run.getFormat()!=null && run.getFormat().getClass()==aFormat.getClass())
                xstring.setAttribute(RMTextTypes.TEXT_FORMAT, aFormat, run.start(), run.end());
        }
    }
...
public static StringBuffer getImageMap(RMShape aShape, StringBuffer aSB)
{
    ...
    if(aShape.getURL()!=null) {
        RMRect bounds = aShape.convertRectToShape(aShape.getBoundsInside(), aShape.getPageShape());
...
public static void passwordReport()
{
    ...
    try { RMFileUtils.writeBytes(new java.io.File("/Users/jeff/Desktop/Test2.pdf"), rm.getBytes(report)); }
...
public static class Movie {
    ...
    public List getStudio2()  { return Arrays.asList(RMMapUtils.newMap("Studio", studio, "ListType", "XJ-27")); }
