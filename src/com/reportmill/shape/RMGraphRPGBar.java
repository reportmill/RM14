...
private void addGrid()
{
    ...
    if(_graph.getValueAxis().getShowMajorGrid())
        for(int i=0, iMax=getIntervalCount(); i<iMax; i++)
            zeroAxisLineAdded = zeroAxisLineAdded || RMMath.equalsZero(getInterval(i));
...
private void addValueAxisLabels()
{
    ...
    for(int i=0, iMax=getIntervalCount(); i<iMax; i++) { Float interval = getInterval(i);
        ...
        RMPoint point2 = isVertical()? new RMPoint(-5, intervalPosition) : new RMPoint(intervalPosition, height + 5);
        ...
        RMPoint point1 = label.getBoundsInside().getPerimeterPointForRadial(angle, true);
        point1 = label.convertPointToShape(point1, label.getParent());
    ...
    // Outside Bottom
        ...
        if(label.getRoll()!=0) {
            ...
            if(isVertical()) {
                ...
                p = label.convertPointToShape(p, label.getParent());
                labelX = labelX - (p.x - barRect.getMidX());
                labelY = labelY - (p.y - barRect.getMaxY()) + label.getHeight()/2*RMMath.cos(label.getRoll()) + 3;
            }
