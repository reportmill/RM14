...
{
    // Get XML element for object, userinfo and degree of separation
    XMLElement rootElement = getXML(anObj, userInfo, degreeOfSeparation);
    if(rootElement==null)
        return;
    ...
    try { RMFileUtils.writeBytes(new java.io.File(aPath), bytes); }
...
public XMLElement getXML(Object anObj, Object userInfo, int degreeOfSeparation)
{
    ...
    return getXML(anObj, null, degreeOfSeparation);
}

/**
 * Returns an element tree for given collections map and schema.
 */
public XMLElement getXML(Object anObj, Schema aSchema, int aDegreeOfSeparation)
{
    // Set schema - or create it with schema maker if not provided
    _schema = aSchema;
    if(_schema==null)
        _schema = getSchemaMaker().getSchema(anObj, "Root", aDegreeOfSeparation);
...
private void writeXML(Object anObject, XMLElement anElement, Entity anEntity)
{
    ...
    int processedIndex = RMListUtils.indexOfId(_processedMaps, anObject);
    if(processedIndex>=0) {
