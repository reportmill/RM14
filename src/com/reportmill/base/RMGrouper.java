 ...
 */
public class RMGrouper extends SnapObject implements PropertyChangeListener, XMLArchiver.Archivable {
...
public void addGrouping(RMGrouping aGrouping, int anIndex)
{
    ...
    aGrouping.addPropertyChangeListener(this);
    
    // Set selected grouping index to new grouping
    _selectedGroupingIndex = anIndex;
    
    // Fire property change
    firePropertyChange("Grouping", null, aGrouping, anIndex);
}
...
{
    return removeGrouping(RMListUtils.indexOfId(_groupings, aGrouping))!=null;
}
...
{
    for(int i=0, iMax=getListenerCount(PropertyChangeListener.class); i<iMax; i++)
        getListener(PropertyChangeListener.class, i).propertyChange(anEvent);
}
...
public RMGrouper clone()
{
    ...
    // Clone deep grouping
    clone._groupings = RMListUtils.cloneDeep(_groupings);
