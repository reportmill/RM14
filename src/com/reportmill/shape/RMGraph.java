...
public void setType(Type aType)
{
    ...
    firePropertyChange("Type", _type, _type=aType, -1);
    relayout();
}
...
public void setSectionLayout(SectionLayout aLayout)
{
    ...
    firePropertyChange("SectionLayout", _sectionLayout, _sectionLayout = aLayout, -1);
    relayout(); repaint();
}
...
public void setItemsLayout(ItemLayout aLayout)
{
    ...
    firePropertyChange("ItemsLayout", _itemsLayout, _itemsLayout = aLayout, -1);
    relayout(); repaint();
}
...
private RMScene3D create3D()
{
    ...
    p3d.addPropertyChangeListener(this); return p3d;
}
...
{
    firePropertyChange("Draw3D", _draw3D, _draw3D = aFlag, -1);
    relayout();
}
...
public void setShowLegend(boolean aFlag)
{
    ...
    if(aFlag && legend==null && getParent()!=null) {
        ...
        legend.setEffect(new RMShadowEffect(new RMColor(0, 0, 0, .65f), 5, 5, 5));
...
public void propertyChange(PropertyChangeEvent anEvent)
{
    ...
    if(!RMListUtils.containsId(getChildren(), anEvent.getSource()))
...
public void paintShapeOver(RMShapePainter aPntr)
{
    ...
    // Get font/metrics
    if(_font==null) _font = new Font("Lucida Grande", Font.PLAIN, 10);
    
    // Get FontBoxes
    aPntr.setFont(_font); aPntr.setStroke(RMAWTUtils.Stroke1);
    _fboxes = new FontBox[getPartCount()]; double x = 3, h = 14;
    for(int i=0, iMax=getPartCount(); i<iMax; i++) { String string = getPartName(i);
        Rectangle2D fbox = aPntr.getStringBounds(string); double w = fbox.getWidth(), fa = aPntr.getFontAscent();
    ...
    for(int i=0, iMax=getPartCount(); i<iMax; i++) { String part = getPartName(i); FontBox fbox = getFontBox(i);
        boolean isSel = aPntr.isSelected(getPart(i));
        Color c = RMColor.lightBlue.awt().brighter().brighter(); if(isSel) c = c.brighter();
        aPntr.setFont(_font); if(isSel) aPntr.setFont(_font.deriveFont(Font.BOLD));
        aPntr.setColor(c); aPntr.fill(fbox);
        aPntr.setColor(Color.black); aPntr.draw(fbox);
        aPntr.drawString(part, (float)fbox.getTextX(), (float)fbox.getTextY());
    }
}
 ...
 */
public static class FontBox extends Rectangle2D.Double {
    double _tx, _ty;
...
public RMGraph clone()
{
    ...
    clone._series = RMListUtils.cloneDeep(_series);
    return clone;
}
