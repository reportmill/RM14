...
public class SnapUtils {
    ...
    // Whether RM is currently running on Windows
    public static boolean isWindows = (System.getProperty("os.name").indexOf("Windows") >= 0);
    
    // Whether RM is currently running on Mac
    public static boolean isMac = (System.getProperty("os.name").indexOf("Mac OS X") >= 0);
    
    // Whether RM is currently running as Swing App
    public static boolean isApp = false;
    
    // Whether RM is currently running as Swing Applet
    public static boolean isApplet = false;
    
    // Whether RM is currently running on Java 1.5+
    public static boolean isJava5 = System.getProperty("java.version").compareTo("1.5.0")>=0;
...
public static boolean boolValue(Object anObj)
{
    ...
    else if(anObj instanceof Number) return !SnapMath.equalsZero(((Number)anObj).floatValue());
...
public static String stringValue(Object anObj)
{
    ...
    if(anObj instanceof Class) return ClassUtils.getClassStandardName(anObj, false);
...
public static InputStream getInputStream(Object aSource)
{
    ...
    System.err.println("RMUtils.getInputStream: Couldn't get stream for " + aSource);
    return null;
}
 ...
 */
public static Method getMethod(Object anObj, String aName, Class ... theParamTypes)
{
    Class cls = anObj instanceof Class? (Class)anObj : anObj.getClass(); if(cls==null) return null;
    try { return cls.getMethod(aName, theParamTypes); }
...
{
    if(!SnapUtils.isApp && !SnapUtils.isApplet)
