 ...
 * Removes a child from this layer.
 */
public int removeChild(RMShape aChild)  { return RMListUtils.removeId(_children, aChild); }
 ...
 */
public int getChildIndex(RMShape aChild)  { return RMListUtils.indexOfId(getChildren(), aChild); }
...
public void bringShapesToFront(List shapes)
{
    ...
    for(int i=0, iMax=shapes.size(); i<iMax; i++) {
        ...
        if(RMListUtils.removeId(_children, child)>=0)
            _children.add(child);
    }
}
...
public Object clone()
{
    ...
    clone._children = RMUtils.clone(_children);
    return clone;
}
