 ...
 */
public void init(Map encryptDict, List fileID, String pdfversion, String uPass) throws PDFBadPasswordException
{
    ...
    try {
        ...
        if(Float.parseFloat(pdfversion.substring(6)) < 1.6)
...
public byte[] getEncryptionKey(Map encrypt, List fileID, String uPass)
{
     ...
     // Set FileId
     if(fileID.size()>0)
         _fileID = RBCodec.bytesForASCIIHex((String)(fileID.get(0)));
...
public static byte[] getBytesForEncryptionEntry(Map encDict, String key)
{
    ...
    if((s.charAt(0)=='<') && (s.charAt(slen-1)=='>'))
        return RBCodec.bytesForASCIIHex(s);
