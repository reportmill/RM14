...
public static XMLElement getElement(Object aSource)
{
    ...
    try { return new XMLElementParser().parse(aSource); }
...
{
    // Create attributes list if absent
    if(_attributes==null) _attributes = new ArrayList();

    // Add attribute at given index
    _attributes.add(anIndex, anAttribute);
}
 ...
 */
public List <XMLElement> getElements()  { return _elements!=null? _elements : (_elements = new ArrayList()); }
...
{
    // If attribute is element, add to elements list
    if(anAttribute instanceof XMLElement)
        addElement((XMLElement)anAttribute);
    else addAttribute(anAttribute);
    return this;
}
...
{
    // Write element preamble
    writePreamble(aSB, indent, indentStr);
    ...
    for(int i=0, iMax=getAttributeCount(); i<iMax; i++) {
        aSB.append(' ');
        aSB.append(getAttribute(i).getName());
        aSB.append("=\"");
        write(aSB, getAttribute(i).getValue());
        aSB.append('"');
    }
    ...
    // If element value, close start tag, add element value and add end tag
        ...
        else write(aSB, getValue());
            
        // Append closing tag
        aSB.append("</"); aSB.append(_name); aSB.append(">\n");
    }
    ...
    // Append child element value
        ...
        for(int i=0, iMax=size(); i<iMax; i++)
            get(i).write(aSB, indent+1, indentStr);
        for(int i=0; i<indent; i++) aSB.append(indentStr);
        aSB.append("</"); aSB.append(_name); aSB.append(">\n");
    }
}
...
/**
 * Write String to StringBuffer (converting chars to XML compliant chars).
 */
private void write(StringBuffer aSB, String aString)
{
