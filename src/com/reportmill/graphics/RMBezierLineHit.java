...
public static RMHitInfo getHitInfo(RMBezier aBezier, RMLine aLine, int which)
{
    ...
    if(RMMath.equals(aLine._sp.y, aLine._ep.y)) {
        C1y[0] -= aLine._sp.y;
        return intersectOneDimension(C1y, C1x, aLine._sp.x, aLine._ep.x, which);
    }
    
    // If vertical line, same as above, but use the x curve and y extent
    else if (RMMath.equals(aLine._sp.x, aLine._ep.x)) {
        C1x[0] -= aLine._sp.x;
        return intersectOneDimension(C1x, C1y, aLine._sp.y, aLine._ep.y, which);
    }
    ...
    // the non-rotated ones.
        ...
        double angle = Math.atan2(aLine._ep.y-aLine._sp.y, aLine._ep.x-aLine._sp.x);
        
        // Get transform to horizontal
        AffineTransform rot = AffineTransform.getRotateInstance(-angle);
        
        // Get rotated line
        RMPoint lp1 = (RMPoint)rot.transform(aLine._sp, new RMPoint());
        RMPoint lp2 = (RMPoint)rot.transform(aLine._ep, new RMPoint());
        RMLine line2 = new RMLine(lp1, lp2);
        
        // Get rotated bezier
        RMPoint bp1 = (RMPoint)rot.transform(aBezier._sp, new RMPoint());
        RMPoint bp2 = (RMPoint)rot.transform(aBezier._cp1, new RMPoint());
        RMPoint bp3 = (RMPoint)rot.transform(aBezier._cp2, new RMPoint());
        RMPoint bp4 = (RMPoint)rot.transform(aBezier._ep, new RMPoint());
        RMBezier bez2 = new RMBezier(bp1, bp2, bp3, bp4);
        
        // Check that it did what we expected so we don't go into infinite recursion
        if(!RMMath.equals(line2._sp.y, line2._ep.y)) {
            
            // If line is almost horizontal, close enough for me
            if(Math.abs(line2._sp.y-line2._ep.y)<.01)
                line2._ep.y = line2._sp.y;
...
{
    double cx = 3*(aBezier._cp1.x - aBezier._sp.x);
    double bx = 3*(aBezier._cp2.x - aBezier._cp1.x) - cx;
    double ax = aBezier._ep.x - aBezier._sp.x-bx-cx;
    double coeffs[] = new double[4];
    coeffs[3] = ax;
    coeffs[2] = bx;
    coeffs[1] = cx;
    coeffs[0] = aBezier._sp.x;
    return coeffs;
}
...
{
    double cy = 3*(aBezier._cp1.y - aBezier._sp.y);
    double by = 3*(aBezier._cp2.y - aBezier._cp1.y) - cy;
    double ay = aBezier._ep.y - aBezier._sp.y-by-cy;
    double coeffs[] = new double[4];
    coeffs[3] = ay;
    coeffs[2] = by;
    coeffs[1] = cy;
    coeffs[0] = aBezier._sp.y;
    return coeffs;
}
