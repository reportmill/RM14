...
public static void main(String args[])
{
    ...
    for(int i=0; i<args.length; i++) { String arg = args[i];
        ...
        if(RMStringUtils.endsWithIC(arg, ".rpt")) rptfile = arg;
            
        // Check for infile (.xml)
        if(RMStringUtils.endsWithIC(arg, ".xml")) infile = arg;
            
        // Check for outfile (.pdf, .swf, .html, .csv)
        if(RMStringUtils.endsWithIC(arg, ".pdf") || RMStringUtils.endsWithIC(arg, ".swf") ||
            RMStringUtils.endsWithIC(arg, ".html") || RMStringUtils.endsWithIC(arg, ".csv"))
            outfile = arg;
        ...
        // Check for test
        if(arg.equals("-test")) {
            count = i+1==args.length? 100 : RMStringUtils.intValue(args[++i]);
        ...
        if(arg.equals("-threads") && i+1<args.length)
            threads = RMStringUtils.intValue(args[++i]);
        ...
        if(arg.startsWith("-fonts")) {
            String fonts[] = arg.equals("-fonts")? RMFontUtils.getFontNames() : RMFontUtils.getFamilyNames();
    ...
    if(license!=null) {
        if(RMUtils.checkString(license, false)) {
            ReportMill.setLicense(license, true, false);
            System.err.println("License is valid and has been installed on host " + RMUtils.getHostname() +
    ...
    if(paginate==null && outfile!=null)
        paginate = new Boolean(RMStringUtils.endsWithIC(outfile, "pdf"));
...
static class RPGThread extends Thread {
    ...
    public void run() {
        for(int i=1; i<=count; i++) {
            ...
            String out = RMStringUtils.replace(outfile, ".", "_" + id + "_" + (i<10?"0":"") + i + ".");
            report.write(out);
