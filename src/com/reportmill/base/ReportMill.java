...
public class ReportMill {
    ...
    private static boolean _licensed = RMUtils.checkString(getLicense(), RMUtils.isApp);
    
/** Static method prints initialization message (when RM invoked by Shell or by other app). */
static
{
    // If not app or applet, print initialization message and turn on headless
...
public static String getLicense()
{
    ...
    if(_license==null) try {
        ...
        String prefsKey = RMUtils.isApp? "HostProperties1" : "HostProperties2";
 ...
 */
public static void setLicense(String aLicense)  { setLicense(aLicense, false, RMUtils.isApp); }
...
public static void lc(RMDocument aDoc)
{
    ...
    if(!_licensed) {
        ...
        if(!RMUtils.isApp) {
            System.err.println("Warning: Unlicensed copy of ReportMill for host " + RMUtils.getHostname() +
...
private static void addWatermark(RMParentShape aShape)
{
    ...
    RMFont font72 = RMFont.getFont("Arial Bold", 72), font100 = RMFont.getFont("Arial Bold", 100);;
    RMColor color = new RMColor(.9f, .9f, .9f);
    RMXString xstring = new RMXString("REPORTMILL", font72, color);
    xstring.setAttribute(font100, 0, 1); // R in 100pt
    xstring.setAttribute(font100, 6, 7); // M in 100pt
