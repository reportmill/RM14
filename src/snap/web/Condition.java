...
{
    if(SnapUtils.equals(aValue, _propertyName)) return; // If value already set, just return
    firePropertyChange(PropertyName_Prop, _propertyName, _propertyName = aValue, -1);
}
...
{
    if(SnapUtils.equals(anOperator, _operator)) return; // If value already set, just return
    firePropertyChange(Operator_Prop, _operator, _operator = anOperator, -1); // Set value and fire PropertyChange
}
...
{
    if(SnapUtils.equals(aValue, _value)) return; // If value already set, just return
    firePropertyChange(Value_Prop, _value, _value = aValue, -1); // Set value and fire PropertyChange
}
...
{
    if(aValue==_negated) return; // If value already set, just return
    firePropertyChange(Negated_Prop, _negated, _negated = aValue, -1); // Set value and fire PropertyChange
}
...
{
    if(aValue==_ignoreCase) return; // If value already set, just return
    firePropertyChange(IgnoreCase_Prop, _ignoreCase, _ignoreCase = aValue, -1); // Set value and fire PropertyChange
}
...
{
    if(aDateConstraint==_dateConstraint) return; // If value already set, just return
    firePropertyChange(DateConstraint_Prop, _dateConstraint, _dateConstraint = aDateConstraint, -1);
}
