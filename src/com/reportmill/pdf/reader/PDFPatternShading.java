...
{
    Object v = srcFile.resolveObject(dict.get("Shading")); // TODO:BUG - what about shading resources that are pdfstreams?
    ...
    if (v instanceof Map) {
        ...
        shadingDict = ((PDFStream)v).getDictionary();
        patternDict = null;
    }
    else {
        shadingDict = dict;
...
public PDFPatternShading(Map patternDict, Map shadingDict, PDFFile srcFile)
{
    ...
    if (patternDict != null) {
        ...
        extGState = (Map)srcFile.resolveObject(patternDict.get("ExtGState"));
    }
    else
        xform = new AffineTransform();
