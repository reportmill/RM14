...
public Settings(WebFile aFile)
{
    ...
    aFile.addPropertyChangeListener(new PropertyChangeListener() { public void propertyChange(PropertyChangeEvent e) {
...
private void readFile()
{
    ...
    _map.clear();
    JSONArchiver archiver = new JSONArchiver(); archiver.addImport("snap.web.*");
    String string = StringUtils.getString(_file.getBytes());
    string = string.replace("SnapSettings", "Settings");
    try { archiver.readObject(string, this); }
...
public void put(String aKey, Object aValue)
{
    ...
    if(!SnapUtils.equals(aValue, value))
        firePropertyChange(aKey, value, aValue, -1);
}
...
{
    if(_parent!=null) _parent.firePropertyChange(_key + '.' + aName, oldVal, newVal, anIndex);
    else {
        super.firePropertyChange(aName, oldVal, newVal, anIndex);
...
public class SettingsList<E> extends AbstractList<E> {
    ...
    public void add(int anIndex, E anItem)
    {
        ...
        firePropertyChange(_key, null, anItem, anIndex);
    }
    ...
    public E remove(int anIndex)
    {
        ...
        firePropertyChange(_key, item, null, anIndex);
        return item;
    }
