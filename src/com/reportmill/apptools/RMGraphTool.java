...
{
    JTabbedPane tpane = (JTabbedPane)super.createUI(); //tpane.setFont(new Font("Lucida Grande", Font.PLAIN, 11));
    tpane.addTab("Bars", _barTool.getUI()); //tpane.addTab("Pie", _pieTool.getUI());
...
protected void initUI()
{
    ...
    ((JComponent)getUI().getComponent(0)).add(_sortPanel.getUI());
    
    // Set values in SectionLayoutList and ItemsLayoutList
    setNodeItems("SectionLayoutList", RMGraph.SectionLayout.values());
    setNodeItems("ItemsLayoutList", RMGraph.ItemLayout.values());
}
...
protected void resetUI()
{
    ...
    JTabbedPane tpane = getUI(JTabbedPane.class); boolean isPie = graph.getType()==RMGraph.Type.Pie;
    JComponent tui = isPie? _pieTool.getUI() : _barTool.getUI(); String str = isPie? "Pie" : "Bar";
    if(tpane.getComponentAt(2)!=tui) { tpane.removeTabAt(2); tpane.insertTab(str, null, tui, str + " Attributes", 2); }
    
    // Update ListKeyText, FilterText
    setNodeValue("ListKeyText", graph.getDatasetKey());
    setNodeValue("FilterText", graph.getFilterKey());
    
    // Update KeysText
    StringBuffer sb = new StringBuffer();
    for(int i=0; i<graph.getKeyCount();i++) { if(i>0) sb.append(", "); sb.append(graph.getKey(i)); }
    setNodeValue("KeysText", sb.toString());
    
    // Set ValuesButton visible/invisible 
    _sortPanel.getNode("ValuesButton", JToggleButton.class).setVisible(graph.getGrouping().getKey()!=null);

    // Update BarVGraphButton, BarHGraphButton, PieGraphButton
    setNodeValue("GraphTypeBarButton", graph.getType()==RMGraph.Type.Bar);
    setNodeValue("GraphTypeBarHButton", graph.getType()==RMGraph.Type.BarH);
    setNodeValue("GraphTypeAreaButton", graph.getType()==RMGraph.Type.Area);
    setNodeValue("GraphTypeLineButton", graph.getType()==RMGraph.Type.Line);
    setNodeValue("GraphTypeScatterButton", graph.getType()==RMGraph.Type.Scatter);
    setNodeValue("GraphTypePieGraphButton", graph.getType()==RMGraph.Type.Pie);
    
    // Reset SectionLayoutList and ItemsLayoutList
    setNodeValue("SectionLayoutList", graph.getSectionLayout());
    setNodeValue("ItemsLayoutList", graph.getItemsLayout());

    // Get the color dock, reset colors and update
    ColorDock colorDock = getNode("ColorDock", ColorDock.class);
    List colors = graph.getColors();
    colorDock.resetColors();
    for(int i=0, iMax=Math.min(colors.size(), colorDock.getSwatchCount()); i<iMax; i++)
        colorDock.setColor(((RMColor)colors.get(i)).awt(), i); 
    
    // Update ShowLegendCheckBox, Draw3DCheckBox
    setNodeValue("ShowLegendCheckBox", graph.getShowLegend());
    setNodeValue("Draw3DCheckBox", graph.getDraw3D());
...
public void respondUI(SwingEvent anEvent)
{
    ...
    if(anEvent.equals("ListKeyText"))
        graph.setDatasetKey(RMStringUtils.delete(anEvent.getStringValue(), "@"));
    
    // Handle FilterText
    if(anEvent.equals("FilterText"))
        graph.setFilterKey(RMStringUtils.delete(anEvent.getStringValue(), "@"));
    ...
    if(anEvent.equals("KeysText")) {
        ...
        String keysString = getNodeStringValue("KeysText");
    ...
    // Handle ColorDock
    if(anEvent.equals("ColorDock")) { ColorDock colorDock = anEvent.getTarget(ColorDock.class);
        int index = colorDock.getSelectedIndex(); // Get the selected index
        ...
        colors.set(index, new RMColor(colorDock.getColor())); // Set color of selected index, remove trailing whites
 ...
 */
public RMGraph getSelectedGraph()  { return RMClassUtils.getInstance(getSelectedShape(), RMGraph.class); }
 ...
 * Handle MousePressed.
 */
public void mousePressed(T aGraph, MouseEvent anEvent)
{
    // See if point hit Graph part
    Point2D point = getEditor().convertPointToShape(anEvent.getPoint(), aGraph);
    for(int i=0, iMax=aGraph.getPartCount(); i<iMax; i++)
        if(aGraph.getFontBox(i).contains(point))
    ...
    if(_hitChild==null && isSuperSelected(aGraph) && aGraph.getDraw3D()) {
        ...
        _hitChild.mousePressed(new RMShapeMouseEvent(_hitChild, anEvent));
 ...
 */
public void mouseDragged(T aGraphArea, MouseEvent anEvent)
{
    // If child is scene3d, forward mouse event to child and consume event
    if(_hitChild instanceof RMScene3D)
        _hitChild.mouseDragged(new RMShapeMouseEvent(_hitChild, anEvent));
 ...
 */
public void mouseReleased(T aGraph, MouseEvent anEvent)
{
    if(_hitChild instanceof RMScene3D)
        _hitChild.mouseReleased(new RMShapeMouseEvent(_hitChild, anEvent));
