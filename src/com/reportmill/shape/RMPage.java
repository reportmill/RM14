...
public void setDatasetKey(String aKeyPath)
{
    ...
    if(RMUtils.equals(aKeyPath, _datasetKey)) return;
    
    // Set value and fire property change
    firePropertyChange("DatasetKey", aKeyPath, _datasetKey = RMStringUtils.delete(aKeyPath, "@"), -1);
}
...
public void setPaintBackground(boolean aValue)
{
    ...
    // Set value, fire property change
    firePropertyChange("PaintBackground", _paintBackground, _paintBackground = aValue, -1);
}
...
public void addLayer(RMPageLayer aLayer, int anIndex)
{
    ...
    if(RMListUtils.containsId(_layers, aLayer)) return;
    ...
    firePropertyChange("Layer", null, aLayer, anIndex);
...
public RMPageLayer removeLayer(int anIndex)
{
    ...
    // Fire property change
    firePropertyChange("Layer", layer, null, anIndex);
...
{
    int index = RMListUtils.indexOfId(_layers, aLayer);
    if(index>=0)
        removeLayer(index);
    return index;
}
...
public void setPlexing(Plexing aPlexing)
{
    ...
    // Set value and fire property change
    firePropertyChange("Plexing", _plexing, _plexing = aPlexing, -1);
}
...
protected boolean isShowing(RMShape aChild)
{
    ...
    for(RMPageLayer layer : getLayers())
        if(RMListUtils.contains(layer.getChildren(), aChild))
...
protected void setChildAnimator(RMAnimator anAnimator)
{
    ...
    if(_childAnimator!=null) _childAnimator.removePropertyChangeListener(this);
    _childAnimator = anAnimator;
    if(_childAnimator!=null) _childAnimator.addPropertyChangeListener(this);
...
public void paintShape(RMShapePainter aPntr)
{
    ...
    // Get clip bounds
    Rectangle2D clipBounds = aPntr.getClipBounds();
    
    // Get bounds we need to draw
    Rectangle2D drawBounds = bounds.createIntersection(clipBounds);
    ...
    if(getPaintBackground() && getStroke()==null) {
        aPntr.setStroke(RMAWTUtils.Stroke1); aPntr.setColor(Color.black);
        aPntr.drawRect(-1, -1, getWidth()+1, getHeight()+1);
    }
    
    // Draw grid if needed
    if(getDocument().getShowGrid() && aPntr.isEditing()) {
        ...
        aPntr.setColor(new Color(13/15f, 13/15f, 13/15f)); // gridColor = new Color(13/15f, 13/15f, 13/15f)
    
        // Draw vertical lines
        for(double x=RMMath.ceil(drawBounds.getX()-.001,gs); x<drawBounds.getMaxX(); x+=gs)
            aPntr.drawLine((int)x, (int)drawBounds.getY(), (int)x, (int)drawBounds.getMaxY()-1);
    
        // Draw horizontal lines
        for(double y=RMMath.ceil(drawBounds.getY()-.001, gs); y<drawBounds.getMaxY(); y+=gs)
    ...
    if(getDocument().getShowMargin() && aPntr.isEditing()) {
        aPntr.setColor(new Color(9/15f, 9/15f, 9/15f)); // marginColor = new Color(9/15f, 9/15f, 9/15f)
