...
public RMNumberFormat()
{
    ...
    // Get FormatSymbols
    _formatSymbols = getDecimalFormatSymbols();
}

/**
 * Creates a number format for the given format string.
 */
public RMNumberFormat(String aFormatString)  { this(); setFormatString(aFormatString); }
...
public RMNumberFormat(RMNumberFormat aFormat) 
{
    ...
    // Copy FormatString, NegativeInRed and NullString
    setFormatString(aFormat.getFormatString());
    setNegativeInRed(aFormat.isNegativeInRed());
    setNullString(aFormat.getNullString());
}
...
public String getLocalCurrencySymbol()
{
    ...
    for(int i=0; i<separators.length; ++i)
        if(separators[i]!=null && separators[i].length()>0)
            symbol = RMStringUtils.replace(symbol, separators[i], ' ');
...
public void setLocalCurrencySymbolUsed(boolean aFlag)
{
    ...
    if(!isLocalCurrencySymbolUsed() && aFlag) {
        ...
        pattern = RMStringUtils.replace(pattern, ";", ";" + currencySymbol + " ");
    ...
    // If currency symbol is used and not wanted, remove it
        ...
        for(int i=0; i<patterns.length; i++) {
            patterns[i] = RMStringUtils.delete(patterns[i], "$");
            patterns[i] = RMStringUtils.delete(patterns[i], currencySymbol);
 ...
 */
public String getThousandsSeparator()  { return isGroupingUsed()? "" + _formatSymbols.getGroupingSeparator() : null; }
...
public void setThousandsSeparator(String aValue)
{
    if(aValue!=null && aValue.length()>0) {
        ...
        _formatSymbols.setGroupingSeparator(c);
        setDecimalFormatSymbols(_formatSymbols);
        setGroupingUsed(true);
        setGroupingSize(3);
    }
    else setGroupingUsed(false);
}

/**
 * Returns the decimal separator.
 */
public String getDecimalSeparator()  { return "" + _formatSymbols.getDecimalSeparator(); }
...
public void setDecimalSeparator(String aValue)
{
    if(aValue!=null && aValue.length()>0) {
        char c = aValue.charAt(0);
        _formatSymbols.setDecimalSeparator(c);
        setDecimalFormatSymbols(_formatSymbols);
    }
}
...
public String getFormatString()
{
    ...
    if(_formatSymbols.getDecimalSeparator()!='.' || (isGroupingUsed() && _formatSymbols.getGroupingSeparator()!=',')) {
        if(isGroupingUsed())
            p = RMStringUtils.replace(p, ",", "xxx");
        p = RMStringUtils.replace(p, ".", "" + _formatSymbols.getDecimalSeparator());
        if(isGroupingUsed())
            p = RMStringUtils.replace(p, "xxx", "" + _formatSymbols.getGroupingSeparator());
    }
...
public void setFormatString(String aFormat)
{
    ...
    _negativeInRed = aFormat.indexOf("[Red]")>0;
    if(_negativeInRed)
    ...
    int length = aFormat.indexOf(';'); if(length<0) length = aFormat.length();
    for(int i=length-1; i>1; i--) {
        char c1 = aFormat.charAt(i), c2 = aFormat.charAt(i-1), c3 = aFormat.charAt(i-2);
        if((c1=='0' || c1=='#') && (c2!='0' && c2!='#' && c2!=' ') && (c3=='0' || c3=='#')) {
            if(gsep!=0 && dsep==0) {
                dsep = gsep; gsep = c2; break; }
            else if(dsep==0 && c3=='0')
                dsep = c2;
            else if(gsep==0)
                gsep = c2;
    ...
    if(gsep>0 && gsep!=',')
         format = RMStringUtils.replace(aFormat, "" + gsep, "xxx");
    if(dsep>0 && dsep!='.')
        format = RMStringUtils.replace(format, "" + dsep, ".");
    if(gsep>0 && gsep!=',')
        format = RMStringUtils.replace(format, "xxx", ",");
    ...
    try {
        ...
        _formatSymbols = getDecimalFormatSymbols();
        if(dsep>0 && dsep!='.')
            _formatSymbols.setDecimalSeparator(dsep);
        if(gsep>0 && gsep!=',')
            _formatSymbols.setGroupingSeparator(gsep);
        setDecimalFormatSymbols(_formatSymbols);
    }
...
public Object formatRM(Object anObj)
{
    ...
    if(anObj instanceof Number) { Number number = (Number)anObj;

        // Round number before it gets to Java, so we don't get "-0"
        double value = number.doubleValue();
        if(value<0 && value>-1) {
            int dp = getMaximumFractionDigits();
            double dpm = Math.pow(10, dp);
            number = Math.round(value*dpm)/dpm;
        }
    
        // For legacy reasons, percent formats are divided by 100. Java formats don't do this - maybe it can go one day.
        if(getPositiveSuffix().indexOf('%') >= 0)
            number = number.doubleValue()/100;
        
        // Do normal DecimalFormat format
        Object formatted = format(number);
        
        // If number is negative and _negativeInRed is true, put formatted string in red xstring
        if(number.doubleValue()<0 && _negativeInRed)
             formatted = new RMXString(formatted.toString(), RMColor.red);
...
public Object fromXML(XMLArchiver anArchiver, XMLElement anElement)
{
    ...
    setFormatString(anElement.getAttributeValue("pattern"));
