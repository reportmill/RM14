 ...
 */
public class XMLElementParser extends DefaultHandler implements org.xml.sax.ext.LexicalHandler {
...
{
    // Get current stack parent element
    XMLElement parentElement = ListUtils.getLast(_estack);
    ...
    // Add element to parentElement elements
    if(parentElement!=null)
        parentElement.add(element);
 
    // Iterate over attributes and add them
    for(int i=0, iMax=attr.getLength(); i<iMax; i++ )
        element.add(attr.getLocalName(i), attr.getValue(i));
}
