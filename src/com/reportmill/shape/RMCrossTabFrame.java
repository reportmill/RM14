...
public void paintShape(RMShapePainter g)
{
    ...
    g.setColor(Color.black); g.setStroke(RMAWTUtils.Stroke1); g.draw(path);
    g.setColor(new Color(11/12f, 11/12f, 11/12f)); g.fill(path);

    // Draw CrossTab button
    RMRect button = new RMRect(getWidth() - 100, getHeight() - 18, 100, 18);
    g.setColor(Color.lightGray); g.fill(button);
    g.setColor(Color.black); g.setStroke(RMAWTUtils.Stroke1); g.draw(button);
    g.setColor(Color.darkGray); g.setFont(RMAWTUtils.HelveticaBold12);
    g.drawString("CrossTab", (float)button.x + 14, (float)button.getMaxY() - 4);
