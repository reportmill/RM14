...
public void setOrigin(RMPoint pt)  { _sheetOrigin = pt; }
...
public ExcelPoint convertPoint(RMPoint pt)
{
    int r, c, dx, dy;
    ...
    float y = (float)Math.max(pt.y-_sheetOrigin.y, 0);
    for(r=0; r<65280; ++r) {
        row = _sheet.getRow(r);
        if (row==null) {
    ...
    double x = RMExcelWriter.getCharWidthFromPoints(Math.max(pt.x-_sheetOrigin.x, 0))*256;
    for(c=0; c<255; ++c) {
...
public HSSFShape addNewShape(RMShape aShape, HSSFShapeContainer aParent)
{
    ...
    if (aParent instanceof HSSFPatriarch) {
        ExcelPoint origin = convertPoint(bounds.getOrigin());
        ExcelPoint max = convertPoint(new RMPoint(bounds.getMaxX(), bounds.getMaxY()));
