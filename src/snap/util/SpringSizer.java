...
import snap.util.SpringLayout.SPLayoutInfo;
 ...
 */
public SpringSizer(SpringLayout aLayout)  { _layout = aLayout; _parent = aLayout.getParent(); }
...
public Box[] getChildBoxes()
{
    ...
    // Get parent height
    double pheight = _bh = _layout.getParentHeight(_parent);
    
    // Get boxes
    _cboxes = new Box[_layout.getChildCount(_parent)];
 ...
 */
class Box extends com.reportmill.base.RMRect {
    ...
    public Box(Object aNode)
    {
        SPLayoutInfo li = _layout.getLayoutInfoSP(aNode);
        _asize = _asize0 = _layout.getAutosizing(aNode);
        _bh = _layout.getBestHeight(aNode, _layout.getWidth(aNode));
        setRect(li._x, li._y, li._w, li._h);
    }
