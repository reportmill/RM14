 ...
 * Creates a new parse rule for given name
 */
public ParseRule(Op anOp, ParseRule aPR)  { _op = anOp; _child0 = aPR; }

/**
 * Creates a new parse rule for given name
 */
public ParseRule(Op anOp, ParseRule aPR1, ParseRule aPR2)  { _op = anOp; _child0 = aPR1; _child1 = aPR2; }
 ...
 */
private Map <String,ParseRule> getNamedRules()  { if(_nrules==null) addNamedRule(new HashMap(), this); return _nrules; }

/** Loads the named rules. */
private void addNamedRule(Map <String,ParseRule> aMap, ParseRule aRule)
{
    String name = aRule.getName();
    if(name!=null && aMap.get(name)!=null) return;
    if(name!=null) { aMap.put(name, aRule); aRule._nrules = aMap; }
    if(aRule.getChild0()!=null) addNamedRule(aMap, aRule.getChild0());
    if(aRule.getChild1()!=null) addNamedRule(aMap, aRule.getChild1());
}
