...
public Font getFont(Map fontDict, PDFFile srcfile)
{
    ...
    if (type.equals("/TrueType") || type.equals("/CIDFontType2")) {
        ...
        descriptor = (Map)srcfile.resolveObject(fontDict.get("FontDescriptor"));
        if (descriptor != null) {
            // TrueType fonts are embedded as streams under the key FontFile2
            fobj = srcfile.resolveObject(descriptor.get("FontFile2"));
            // Another possibility is to have FontFile3, with a FontFile2 subtype
            if (fobj==null) {
                fobj = srcfile.resolveObject(descriptor.get("FontFile3"));
                if (fobj instanceof PDFStream)
                    if (!((PDFStream)fobj).getDictionary().get("Subtype").equals("/FontFile2"))
                        fobj=null;
            }
...
/*
{
    FileOutputStream fout = new FileOutputStream("/tmp/embed.ttf");
    fout.write(fontFile);
    fout.flush();
    fout.close();
}
*/
    ...
    if (awtFont == null) {
        String fontName = (String)srcfile.resolveObject(fontDict.get("BaseFont"));
        if (fontName != null) 
...
public Font getFont(String name, String type)
{
    ...
    if (type.equals("/TrueType")) {
        List fontPieces = RMStringUtils.separate(name, ",");
...
{
    //TODO:  Might be able to do a half-assed job using java.awt.font.TextAttributes
    return null;
}
...
public GlyphMapper getGlyphMapper(Map fontDict, PDFFile srcfile)
{
    ...
    // For composite fonts, if the descendant CID font knows how to 
    // map the CIDs to glyphs, add that information to the glyphmapper.
    if ("/Type0".equals(fontDict.get("Subtype"))) {
        Map child = getDescendantFont(fontDict, srcfile);
        mapper.setCIDToGIDMap(srcfile.resolveObject(child.get("CIDToGIDMap")));
    }
...
/** Utility routine for composite fonts.  Composite fonts have a single
 * descendant CID font.
 */
Map getDescendantFont(Map fontDict, PDFFile srcfile)
{
    if ("/Type0".equals(fontDict.get("Subtype"))) {
        List descendants = (List)srcfile.resolveObject(fontDict.get("DescendantFonts"));
        Map child;
        String subtype;
        if (descendants == null)
            throw new PDFException("Can't find descendant for Type 0 composite font "+fontDict);
        child = (Map)srcfile.resolveObject(descendants.get(0));
 ...
 * Return value is either a float[] for simple single-byte fonts
 * or an instance of a PDFGlyphWidthTable for multi-byte or CID
 * fonts.
 */
//public float[] getGlyphWidths(Map fontDict, PDFFile srcfile)
public Object getGlyphWidths(Map fontDict, PDFFile srcfile)
{
    ...
    //CIDFonts look up their widths from a GlyphWidthTable
        ...
        obj = new PDFGlyphWidthTable((List)srcfile.resolveObject(fontDict.get("W")), fontDict.get("DW"));        
    }
    //Single byte fonts use a simple array of 256 floats
        ...
        Map descriptor=(Map)srcfile.resolveObject(fontDict.get("FontDescriptor"));
        if (descriptor != null) {
        ...
        Object pdfw = srcfile.resolveObject(fontDict.get("Widths"));
