 ...
 */
private static class HTMLParser extends HTMLEditorKit.ParserCallback implements RMTextTypes {
    ...
    public HTMLParser(String aString, RMFont baseFont)
    {
        ...
        if(aString.indexOf(';')>aString.indexOf('&')) {
            for(int i=0, iMax=_symMap.length; i<iMax; i++) {
                aString = RMStringUtils.replace(aString, _symMap[i].ce, _symMap[i].map);
                if(aString!=s2) {
                    if(aString.indexOf(';')>aString.indexOf('&'))
                        s2 = aString;
                    else break;
    ...
    public void handleStartTag(HTML.Tag aTag, MutableAttributeSet anAttributeSet, int pos)
    {
        ...
        if(aTag.equals(HTML.Tag.B) || aTag.equals(HTML.Tag.STRONG)) {
            RMFont font = (RMFont)_attrs.get(TEXT_FONT);
            RMFont bold = font.getBold()==null? font : font.getBold();
            _attrs.put(TEXT_FONT, bold);
            _fontStack.add(bold);
        }
        ...
        if(aTag.equals(HTML.Tag.I) || aTag.equals(HTML.Tag.EM)) {
            ...
            _attrs.put(TEXT_FONT, italic);
            _fontStack.add(italic);
        }
        
        // Handle Underline (<U>)
        if(aTag.equals(HTML.Tag.U))
            _attrs.put(TEXT_UNDERLINE, 1);
        ...
        if(aTag.equals(HTML.Tag.UL) || aTag.equals(HTML.Tag.OL)) {
            ...
            RMFont font = (RMFont)_attrs.get(TEXT_FONT);
            RMParagraph ps = getParagraph();
            float leftIndentFirst = ps.getTab(_listLevel-1);
            double leftIndent = leftIndentFirst + font.stringAdvance(((char)8226) + " ");
            ps = ps.deriveIndent(leftIndent, leftIndentFirst, ps.getRightIndent());
            _attrs.put(TEXT_PARAGRAPH, ps);
        }
            ...
            for(Enumeration e=anAttributeSet.getAttributeNames(); e.hasMoreElements();) {
                ...
                if(name.equalsIgnoreCase("color")) {
                    ...
                    if(string!=null) {
                        ...
                        _attrs.put(TEXT_COLOR, color);
                    }
                }
                
                // Handle font size
                if(name.equalsIgnoreCase("size")) {
                    int size = Math.abs(RMStringUtils.intValue(string));
                ...
                if(name.equalsIgnoreCase("face")) {
                    List names = RMStringUtils.separate(string, ",");
                    for(int i=0; i<names.size(); i++) {
                        String n = (String)names.get(i);
                        RMFont f = new RMFont(n, font.getSize());
                        if(!f.isSubstitute()) {
                            if(f.getFontNameEnglish().startsWith("Symbol")) _isSymbol = true; //else
                            font = f;
                            break;
                        }
                    }
        ...
        if(aTag.equals(HTML.Tag.CENTER)) {
            RMParagraph ps = getParagraph().deriveAligned(RMTypes.AlignX.Center);
            _attrs.put(TEXT_PARAGRAPH, ps);
        }
        
        // Handle super-scripting
        if(aTag.equals(HTML.Tag.SUP))
            _attrs.put(TEXT_SCRIPTING, 1);
    ...
    public void handleEndTag(HTML.Tag t, int pos)
    {
        ...
        if(t.equals(HTML.Tag.B) || t.equals(HTML.Tag.STRONG) ||
           t.equals(HTML.Tag.I) || t.equals(HTML.Tag.EM) || t.equals(HTML.Tag.FONT)) {
            if(_fontStack.size()>1) RMListUtils.removeLast(_fontStack);
            RMFont font = (RMFont)RMListUtils.getLast(_fontStack);
            _attrs.put(TEXT_FONT, font);
        ...
        if(t.equals(HTML.Tag.UL) || t.equals(HTML.Tag.OL)) {
            ...
            RMFont font = (RMFont)_attrs.get(TEXT_FONT);
            RMParagraph ps = getParagraph();
            float leftIndentFirst = _listLevel==0? 0 : ps.getTab(_listLevel-1);
            double leftIndent = _listLevel==0? 0 : leftIndentFirst + font.stringAdvance(((char)8226) + " ");
            ps = ps.deriveIndent(leftIndent, leftIndentFirst, ps.getRightIndent());
            _attrs.put(TEXT_PARAGRAPH, ps);
        }
        ...
        if(t.equals(HTML.Tag.CENTER)) {
            if(!_string.getRunLast().toString().endsWith("\n")) _string.addChars("\n");
            RMParagraph ps = getParagraph().deriveAligned(RMTypes.AlignX.Left);
            _attrs.put(TEXT_PARAGRAPH, ps);
        }
                
        // Handle super-scripting
        if(t.equals(HTML.Tag.SUP))
            _attrs.remove(TEXT_SCRIPTING);
    ...
    public void handleText(char data[], int pos)
    {
        ...
        // Add String to text
        _string.addChars(new String(data), _attrs);
    }
    
    /** Returns a symbol char for a given ASCII char. */
    private char getSymbolChar(char aChar)  { return aChar<256? _symChars[aChar] : aChar; }
    
    /** Returns a RM paragraph object for the current set of attributes during parsing.  */
    private RMParagraph getParagraph()
    {
        RMParagraph ps = (RMParagraph)_attrs.get(TEXT_PARAGRAPH);
        if(ps==null) ps = RMParagraph.DEFAULT;
        ps = ps.clone();
        return ps;
    }
}
